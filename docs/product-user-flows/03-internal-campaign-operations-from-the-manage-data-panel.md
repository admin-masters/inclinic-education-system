...
- What the user sees: A searchable campaign table with brand campaign IDs, brand names, company names, dates, and action buttons.
...
- Common issues / trainer notes: The campaign list combines master data and local data; blank start or end dates usually mean the portal-side campaign row is not fully configured yet.
...
- What the user sees: A campaign-specific page where the operator can inspect current metadata and decide which downstream tool to use next.
...
- Common issues / trainer notes: In the current product, the view and edit buttons are usually enough to verify whether a portal campaign row already exists for a master campaign.
- Screenshot placeholder:
...
- What the user sees: A new tab that opens the downstream route with the campaign already filtered.
...
- Common issues / trainer notes: This is where trainers should explain the campaign UUID as the shared handoff key across the product.
- Screenshot placeholder:
  Suggested file path: `docs/product-user-flows/assets/internal-campaign-operations-from-the-manage-data-panel/manage-data-panel.png`
...
Validated against the seeded Manage Data Panel on 2026-04-11.
