...
""",
        ...
        "steps": [
            {
                ...
                "user_sees": "A minimal publisher landing page confirming the campaign ID and offering the `Add details for this campaign` and `Edit details for any other campaign` actions.",
            ...
            },
            {
                ...
                "user_sees": "A campaign form that keeps master-system fields read-only and exposes portal-specific fields such as campaign name, dates, logos, background image, printing requirement, and status, plus `Add Field Rep` and `Edit Field Rep` outbound buttons.",
        ...
        "steps": [
            {
                ...
                "user_sees": "A searchable campaign table with brand campaign IDs, brand names, company names, dates, and row buttons labeled `View`, `Edit`, `Collateral`, and `Field Reps`.",
                ...
                "trainer_notes": "The live-search field debounces and refreshes the URL query string automatically. Blank start or end dates usually mean the portal-side campaign row is not fully configured yet.",
            ...
            },
            {
                ...
                "user_sees": "A campaign detail or campaign update page where the operator can confirm the local description, dates, status, and other portal-side metadata for the selected brand campaign.",
                ...
                "trainer_notes": "Use `View` when you only need a quick read of the campaign, and `Edit` when you need to change the PE-side metadata or inspect the master read-only fields in more depth.",
            ...
            },
            {
                ...
                "user_sees": "A new tab that opens either the collateral dashboard or the field-rep administration list with the same brand campaign ID already carried into the next screen.",
                ...
                "trainer_notes": "This is where trainers should explain the brand campaign ID as the shared handoff key across the product. The `Collateral` button opens the rep-facing collateral dashboard, and `Field Reps` opens the staff rep list.",
                "screenshot_file": "manage-data-panel.png",
        ...
        "steps": [
            {
                ...
                "user_sees": "A table of master field reps with unique ID, field ID, Gmail ID, phone number, campaign mapping, a search box, and row actions for `Edit`, `Delete`, and `View Doctors`.",
                ...
                "trainer_notes": "The list mixes master-table data with a display-friendly campaign mapping string to make the assignment state easy to explain. When a campaign filter is present, the `Add Field Rep` button can redirect to the external field-rep site rather than the local Django form.",
            ...
            },
            {
                ...
                "user_sees": "A standard field-rep form that writes back to the master data tables and preserves the campaign filter in the redirect when the local form is used.",
            ...
            },
            {
                ...
                "user_does": "Use the `View Doctors` action to add a doctor from the inline form or edit/delete existing doctors associated with the rep.",
                "user_sees": "A rep-specific doctor list with a compact add-doctor form at the top and `Edit` and `Delete` buttons beside each doctor row.",
    ...
    },
    {
        ...
        "deck_group": "Publisher and Campaign Operations",
        "document_purpose": "Show how campaign assets, dashboard shortcuts, collateral schedules, and WhatsApp messages are configured before field reps start sharing.",
        ...
        "steps": [
            ...
            },
            {
                ...
                "user_does": "Open the Add Collateral form, choose the campaign and purpose, upload the PDF and banners, choose the type, and add descriptive or webinar metadata before submitting.",
                "user_sees": "A large multipart form for campaign, purpose, content title, content ID, collateral type, PDF or Vimeo input, banners, doctor display name, description, webinar details, and an inline WhatsApp message text area.",
                ...
                "trainer_notes": "The type selector changes which asset inputs are visible: PDF, Vimeo, or both. The submit button saves both the collateral record and the optional WhatsApp message text for that campaign-collateral pair.",
            ...
            },
            {
                ...
                "user_sees": "A searchable message list with campaign and collateral filters, message preview, status, created date, and edit/delete actions.",
            ...
            },
            {
                ...
                "user_does": "Open the Edit Calendar screen from the top dashboard button or the row-level `Edit Dates` action and confirm the start and end dates for the campaign-collateral mapping.",
                "user_sees": "A calendar-edit screen that shows the campaign ID, collateral title, and editable start and end date controls.",
    ...
    },
    {
        ...
        "steps": [
            ...
            },
            {
                ...
                "user_does": "Use the right-side search box, status filter, and the shared collateral selector to find the doctor who needs action for the currently selected collateral.",
                ...
                "trainer_notes": "Changing the collateral dropdown refreshes the page so the doctor-row statuses are recalculated for that asset. The right-side filter is a queue-management aid, not a separate data source.",
            ...
            {
                "number": 4,
                "title": "Send the first collateral message with the Submit flow",
                "user_does": "Complete the left-side doctor form and click `Submit` to send the currently selected collateral to a manually entered doctor.",
            ...
            {
                "number": 5,
                "title": "Use the right-side quick-send buttons and follow-up states",
                "user_does": "Click a doctor-row `Send Message` button for a first send, then revisit the same queue later to use the `Sent` and `Send Reminder` buttons as follow-up shortcuts.",
                "user_sees": "A right-side work queue where first-send and follow-up actions reuse the currently selected collateral and launch the same WhatsApp handoff logic as the left form.",
                "why_it_matters": "This is the fastest day-to-day sharing path once the rep's doctor roster is already loaded.",
                "expected_result": "The rep can send, resend, or remind from the doctor list without retyping the doctor details.",
                "trainer_notes": "The quick-send buttons on the right use the same POST flow as the left form. In the current UI only `Opened` is disabled; `Sent` is still actionable as a resend shortcut, while `Send Reminder` appears once the six-day threshold is reached.",
                "screenshot_file": "fieldrep-send-message.png",
                "screenshot_caption": "Doctor queue filtered to the first-send state with an active Send Message button.",
                "screenshot_focus": "The row-level action that launches the quick-send version of the share flow.",
            },
            {
                "number": 6,
                "title": "Interpret reminder and opened states",
                "user_does": "Return to the same doctor list after a share and use the row labels to decide whether a doctor needs a resend, a reminder, or no action.",
                "user_sees": "A reminder-due queue where older unopened shares show `Send Reminder`, recently shared doctors can still show `Sent`, and engaged doctors show a disabled `Opened` button.",
                "why_it_matters": "This is how reps decide whether to resend, remind, or move on without opening a separate report.",
                "expected_result": "The rep can identify reminder-ready doctors quickly and avoid recontacting doctors who already opened the collateral.",
                "trainer_notes": "Explain the age rule explicitly: the template changes from `Sent` to `Send Reminder` once the latest share is older than six days and there is still no doctor engagement for that collateral.",
    ...
    },
    {
        ...
        "workflow_summary": [
            ...
            "The report header exposes both a collateral filter and visible `Reset` and `Download CSV` controls.",
            ...
            "The doctor table is the operational follow-up surface for identifying which doctors engaged and how far they got, and it includes client-side search plus a status filter.",
        ],
        "steps": [
            {
                "number": 1,
                ...
                "user_sees": "A branded reporting page with a campaign selector, collateral filter, summary section, doctor table, and visible `Reset` and `Download CSV` controls.",
            ...
            },
            {
                ...
                "user_sees": "The same dashboard narrowed to a single collateral's doctor outcomes, with the summary counters and doctor rows recalculated for that asset.",
            ...
            },
            {
                ...
                "user_does": "Use the search box and status filter above the table, then inspect who clicked, viewed, downloaded, or reached the later video buckets.",
                "user_sees": "A doctor table that combines transaction ID, doctor number, collateral ID, clicked state, PDF progress, video progress, transaction date, and updated timestamp.",
                ...
                "trainer_notes": "Use the report right after the doctor-viewer demo so the audience recognizes how those actions appear operationally. The status filter is client-side, so it is safe to use live during training without losing the page state.",
                "screenshot_file": "report-dashboard.png",
            ...
            },
            {
                ...
                "user_sees": "The same report page with a dedicated `Download CSV` button positioned alongside the `Reset` control at the top of the dashboard.",
                ...
                "trainer_notes": "The export action preserves the current collateral filter in the query string so stakeholders can take either the whole campaign dataset or a filtered asset-specific extract.",
                "screenshot_file": "report-dashboard.png",
